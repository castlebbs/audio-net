...
esp_err_t AC101_init(audio_hal_codec_config_t* codec_cfg) {
	...
	// DR Was Mic2 Boost Stage, Now line in res |= AC101_Write_Reg(ADC_SRC, 0x2020);
	res |= AC101_Write_Reg(ADC_SRC, 0x0408);
	res |= AC101_Write_Reg(ADC_DIG_CTRL, 0x8000);
	// DR Remove mic BIAS, enable another ADC Channel res |= AC101_Write_Reg(ADC_APC_CTRL, 0xbbc3);
	...
	//res |= AC101_Write_Reg(OMIXER_SR, 0x0488);
...
esp_err_t AC101_start(ac_module_t mode)
{
	...
	AC101_set_voice_volume(60); // According to spec 63 is maximum.
    }
...
esp_err_t AC101_ctrl_state(audio_hal_codec_mode_t mode, audio_hal_ctrl_t ctrl_state)
{
	...
	switch (mode) {
		case AUDIO_HAL_CODEC_MODE_ENCODE:
			ESP_LOGI(AC101_TAG, "Codec mode AC_MODULE_ADC");
			es_mode_t  = AC_MODULE_ADC;
			break;
		case AUDIO_HAL_CODEC_MODE_LINE_IN:
			ESP_LOGI(AC101_TAG, "Codec mode AC_MODULE_LINE");
			es_mode_t  = AC_MODULE_LINE;
			break;
		case AUDIO_HAL_CODEC_MODE_DECODE:
			ESP_LOGI(AC101_TAG, "Codec mode AC_MODULE_DAC");
			es_mode_t  = AC_MODULE_DAC;
			break;
		case AUDIO_HAL_CODEC_MODE_BOTH:
			ESP_LOGI(AC101_TAG, "Codec mode AC_MODULE_ADC_DAC");
...
esp_err_t AC101_config_i2s(audio_hal_codec_mode_t mode, audio_hal_codec_i2s_iface_t* iface)
{
	...
	case AUDIO_HAL_BIT_LENGTH_16BITS:
		ESP_LOGI(AC101_TAG, "Audio i2s bitlength 16");
		bits = BIT_LENGTH_16_BITS;
		break;
	case AUDIO_HAL_BIT_LENGTH_24BITS:
		ESP_LOGI(AC101_TAG, "Audio i2s bitlength 24");
		bits = BIT_LENGTH_24_BITS;
		break;
	default:
		ESP_LOGI(AC101_TAG, "Audio i2s bitlength 16 (default)");
		bits = BIT_LENGTH_16_BITS;
	}
	...
	case AUDIO_HAL_192K_SAMPLES:
		...
		break;*/
	default:
		ESP_LOGI(AC101_TAG, "Audio i2s samples 44k (default)");
		sample = SIMPLE_RATE_44100;
	}
	ESP_LOGI(AC101_TAG, "Sample: %d  Format: %x", sample, fmat);
...
esp_err_t AC101_set_voice_volume(int volume)
{
	...
	//DR res |= ac101_set_spk_volume(volume);
	ESP_LOGI(AC101_TAG, "Volume set to: %d", volume);
	return res;
}
...
esp_err_t AC101_get_voice_volume(int* volume)
{
	*volume = ac101_get_earph_volume();
	ESP_LOGI(AC101_TAG, "Volume get: %d", *volume);
	return 0;
}
